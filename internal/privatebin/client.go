...
// posting the morning briefing as an encrypted paste and returning the
...
// PostPaste encrypts content and posts it to the PrivateBin instance at
// instanceURL (e.g. "https://privatebin.net"). Returns the full paste URL
// including the decryption key fragment, or an error.
//
// Pastes are set to expire after 24 hours, use the "markdown" formatter, and
// have no password or burn-after-reading.
func PostPaste(ctx context.Context, instanceURL string, content []byte) (string, error) {
	...
	result, err := client.CreatePaste(ctx, content, pb.CreatePasteOptions{
		Formatter:        "markdown",
		Expire:           "1day",
